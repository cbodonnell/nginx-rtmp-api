...
func publishDone(w http.ResponseWriter, r *http.Request) {
	...
	writeLineToFile(fmt.Sprintf("/var/www/hls/%s/%s_low/index.m3u8", vars["name"], vars["name"]), "#EXT-X-ENDLIST")
	writeLineToFile(fmt.Sprintf("/var/www/hls/%s/%s_mid/index.m3u8", vars["name"], vars["name"]), "#EXT-X-ENDLIST")
	writeLineToFile(fmt.Sprintf("/var/www/hls/%s/%s_hi/index.m3u8", vars["name"], vars["name"]), "#EXT-X-ENDLIST")
}
